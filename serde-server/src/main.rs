...
fn main() {
    ...
    if args[1] == "--server" {
        let listener = TcpListener::bind("0.0.0.0:8080").expect("Couldn't bind");
        ...
        println!("Please provide a 3D point as three comma separted integers");
        ...
        loop {
            ...
            stream.write_all(serde_json::to_string(&point).unwrap().as_bytes()).expect("Failed to write to server");
